...
class DECA(nn.Module):
    ...
    def _setup_renderer(self, model_cfg):
        """
        this function used interpolate
            for example:<uv_face_eye
                        face_mask
                        mean_tex>

        which is done by pytorch.
        """
    ...
    def _create_model(self, model_cfg):
        ...
        self.n_detail = model_cfg.n_detail # load on the face exp
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        if return_vis:
            ...
            uv_gt = F.grid_sample(images, uv_pverts.permute(0,2,3,1)[:,:,:,:2], mode='bilinear') # 把输入的tensor转换为指定大小
            print(uv_gt.size())
            print(uv_texture.size())
            if self.cfg.model.use_tex:
